...
Environment.Settings = {
	...
	TriggerKey = "Q", -- 👈 Changeable key here
	Toggle = true,    -- 👈 Toggle ON so you press Q to turn ON/OFF
	LockPart = "Head"
}
    ...
    if not Environment.Locked then
        ...
        for _, v in next, Players:GetPlayers() do
            if v ~= LocalPlayer and v.Character and v.Character:FindFirstChild(Environment.Settings.LockPart) and v.Character:FindFirstChildOfClass("Humanoid") then
                ...
                -- Only lock onto the player if they are within the FOV circle
                if Distance < RequiredDistance and OnScreen then
                    local FOVDistance = (Vector2(Vector.X, Vector.Y) - Vector2(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2)).Magnitude
                    if FOVDistance <= (Environment.FOVSettings.Amount / 2) then
...
local function Load()
		...
		if Running and Environment.Settings.Enabled then
			...
			if Environment.Locked then
				if Environment.Settings.ThirdPerson then
					-- Adjust third-person sensitivity and smooth mouse movement
					...
					-- Regular aimbot with smooth camera transition
	...
	ServiceConnections.InputBeganConnection = UserInputService.InputBegan:Connect(function(Input)
		if not Typing then
			pcall(function()
				if Input.KeyCode == Enum.KeyCode[Environment.Settings.TriggerKey:upper()] then
